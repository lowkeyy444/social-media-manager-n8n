...
//       setPosts((prev) => prev.map((p) => (p._id === id ? { ...p, ...updated, status } : p)));
...
//           Click any post to open full view — approve, reject, copy text, or delete.
//         </p>
//       </header>
...
//                   whileHover={{ scale: 1.02, boxShadow: "0 8px 40px rgba(0,204,255,0.08)" }}
...
//                             setActive(post);
//                           }}
//                           className="text-xs px-2 py-1 rounded-md bg-[rgba(0,204,255,0.06)] hover:bg-[rgba(0,204,255,0.08)] transition"
//                         >
//                           View
//                         </button>
//                       </div>
...
//                       <button
//                         onClick={(e) => { e.stopPropagation(); handleAction(active._id, "approved", false); }}
//                         disabled={isProcessing}
//                         className="px-4 py-2 rounded-lg bg-gradient-to-r from-cyan-400 to-blue-500 text-black font-semibold shadow hover:shadow-[0_0_25px_#00cfff50]"
//                       >
//                         Approve
//                       </button>
//                       <button
//                         onClick={(e) => { e.stopPropagation(); handleAction(active._id, "rejected", false); }}
//                         disabled={isProcessing}
...
export default function ReviewPosts() {
  ...
  const handleAction = async (id, status, closeAfter = false) => {
    ...
    try {
      ...
      // ✅ Changed PATCH → POST so backend can handle posting logic
      const res = await axios.patch(
        "/api/dashboard/reviewpost",
        { postId: id, status },
        { headers: { Authorization: `Bearer ${token}` } }
                  ...
                  <div className="mt-auto pt-4 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleAction(active._id, "rejected", false);
                        }}
                        disabled={isProcessing}
                        className="px-4 py-2 rounded-lg bg-gradient-to-r from-red-400 to-pink-500 text-black font-semibold shadow hover:shadow-[0_0_25px_#ff6b8a50]"
                      >
